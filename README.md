...
## Binance data source (v2)

This version uses Binance's current public C2C Agent market endpoints:
- `/bapi/c2c/v1/public/c2c/agent/trade-methods`
- `/bapi/c2c/v1/public/c2c/agent/ad-list`

Binance documents these as public Phase 1 market queries that do not require authentication.

If the public interface changes, is rate-limited, or returns an unexpected schema, the scanner fails closed rather than inventing prices.


## v3 fixes
- Restored the missing `load_config()` function.
- Scanner failures now fail closed with `UNABLE TO VERIFY — DO NOT TRADE`.
- Telegram alerts are sent for verification failures and trade candidates.
- GitHub Actions updated to current checkout/setup-python major versions.
